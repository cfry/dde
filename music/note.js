...
Note = class Note{
    ...
    static pitch_to_octave(pitch) { return Math.floor(pitch / 12) }
    ...
    static pitch_name_to_number(pitch_name){
        if (typeof(pitch_name) == "number") { return pitch_name }
            ...
            dde_error("fry test error")
            return WebMidi.guessNoteNumber(pitch)
        }
    }
    ...
    static diatonic_transpose(pitch, key, diatonic_interval){
        ...
        diatonic_interval -= 1 //so that 1 will mean "unison, 3 will mean the interval of a 3rd, etc.
