...
//require ( 'three/examples/js/loaders/GLTFLoader' ); //from brad code
var THREE_GLTFLoader = require('three-gltf-loader') //from Fry sim

const OrbitControls	= require ( 'three-orbitcontrols' );

const { dynamics2 }	 = require ( './sim2/dynamics2' );
const { DexterSim2 } = require ( './sim2/dexter' );
...
window.Sim2 = (function() {
	...
	self.createFloor = function() {
		...
		loader.load ( 'sim2/checker.png', texture => {
			const planeSize = 2;
	...
	self.getLinks = function ( grp ) {
		...
		DexterSim2.HiRes.base = self.c0c0.children[0];

		DexterSim2.HiRes.link7 = self.chainLink ( 7 );
		DexterSim2.HiRes.link6 = self.chainLink ( 6 );
		DexterSim2.HiRes.link5 = self.chainLink ( 5 );
		DexterSim2.HiRes.link4 = self.chainLink ( 4 );
		DexterSim2.HiRes.link3 = self.chainLink ( 3 );
		DexterSim2.HiRes.link2 = self.chainLink ( 2 );
		DexterSim2.HiRes.link1 = self.chainLink ( 1 );

		//	Link 7 position.x will open/close the gripper (move the finger).
		//	x = -0.5		appears to be fully open
		//	x =  0.02		fully closed
		let l7 = DexterSim2.HiRes.link7;
		...
		//	x = 0.5		appears to be fully open
		//	x = 0.0		fully closed
		DexterSim2.HiRes.finger = l7.children[0].children[0].children[0];
		...
		self.castShadow ( DexterSim2.HiRes.base );
	
		self.createCoordFrame ( DexterSim2.HiRes.base, 0.50, 0.8, false );
		self.gatherBoundingBoxes ( 0, DexterSim2.HiRes.base );
		//	Default bounding box for the link.
		bbDef = self.defineBoundingBox ( 0, DexterSim2.HiRes.base,
								 'DexterHDI_Link1_KinematicAssembly_v1' );
		config.base = { obj:	DexterSim2.HiRes.base,
						bbDef:	bbDef };

		self.createCoordFrame ( DexterSim2.HiRes.link1, 0.50, 0.8, false );
		self.gatherBoundingBoxes ( 1, DexterSim2.HiRes.link1 );
		bbDef = self.defineBoundingBox ( 1, DexterSim2.HiRes.link1,
								 'HDI-210-001_MainPivot_v461' );
		config.link1 = { obj:	DexterSim2.HiRes.link1,
						 bbDef:	bbDef };

		self.createCoordFrame ( DexterSim2.HiRes.link2, 0.50, 0.8, false );
		self.gatherBoundingBoxes ( 2, DexterSim2.HiRes.link2 );
		bbDef = self.defineBoundingBox ( 2, DexterSim2.HiRes.link2,
								 'DexterHDI_Link2_KinematicAssembly_v5' );
		config.link2 = { obj:	DexterSim2.HiRes.link2,
						 bbDef:	bbDef };

		self.createCoordFrame ( DexterSim2.HiRes.link3, 0.50, 0.8, false );
		self.gatherBoundingBoxes ( 3, DexterSim2.HiRes.link3 );
		bbDef = self.defineBoundingBox ( 3, DexterSim2.HiRes.link3,
								 'HDI_L3Skins_v541' );
		config.link3 = { obj:	DexterSim2.HiRes.link3,
						 bbDef:	bbDef };

		self.createCoordFrame ( DexterSim2.HiRes.link4, 0.50, 0.8, false );
		self.gatherBoundingBoxes ( 4, DexterSim2.HiRes.link4 );
		bbDef = self.defineBoundingBox ( 4, DexterSim2.HiRes.link4,
								 'HDI_DiffSkins_v371' );
		config.link4 = { obj:	DexterSim2.HiRes.link4,
						 bbDef:	bbDef };

		self.createCoordFrame ( DexterSim2.HiRes.link5, 0.50, 0.8, false );
		self.gatherBoundingBoxes ( 5, DexterSim2.HiRes.link5 );
		bbDef = self.defineBoundingBox ( 5, DexterSim2.HiRes.link5,
							//	 'HDI-950-000_GripperCover_v91' );
								 'DexterHDI_Link5_KinematicAssembly_v2' );
		config.link5 = { obj:	DexterSim2.HiRes.link5,
						 bbDef:	bbDef };
		...
		if ( self.bGotStationaryFingerPad ) {
			...
			def = { x:	 0.01,
					...
					h:	 0.026,
					l:	 0.060 };
			self.attachFingerPad ( DexterSim2.HiRes.link6,
								   'Stationary_Finger_Pad', def ); }
		self.gatherBoundingBoxes ( 6, DexterSim2.HiRes.link6 );
		self.createCoordFrame ( DexterSim2.HiRes.link6, 0.50, 0.8, false );
		if ( self.bCreateStationaryFingerPad ) {
			...
			def = { x: self.fpm.x.value,
					...
					h: self.fpm.h.value,
					l: self.fpm.l.value };
			self.createFingerPad ( DexterSim2.HiRes.link6, def ); }
		if ( self.bGotStationaryFingerPad ) {
			bbDef = self.defineBoundingBox ( 6, DexterSim2.HiRes.link6,
								 	 'Stationary_Finger_Pad' ); }
		else {
			bbDef = self.defineBoundingBox ( 6, DexterSim2.HiRes.link6,
									 'CenterGripper_v11' ); }
		config.link6 = { obj:	DexterSim2.HiRes.link6,
						 bbDef:	bbDef,
		...
		if ( self.bGotMovingFingerPad ) {
			def = { x:	-0.17, 
					...
					h:	 0.026,
					l:	 0.060 };
			self.attachFingerPad ( DexterSim2.HiRes.finger,
								   'Moving_Finger_Pad', def ); }
		//	Try self. Note that DexterSim2.HiRes.finger is set above.
		self.gatherBoundingBoxes ( 7, DexterSim2.HiRes.finger );
		self.createCoordFrame ( DexterSim2.HiRes.finger, 0.40, 0.8, false );
		if ( self.bCreateMovingFingerPad ) {
			def = { x: self.fpm.x.value,
					...
					h: self.fpm.h.value,
					l: self.fpm.l.value };
			self.createFingerPad ( DexterSim2.HiRes.finger, def ); }
		if ( self.bGotMovingFingerPad ) {
			bbDef = self.defineBoundingBox ( 7, DexterSim2.HiRes.finger,
							'Moving_Finger_Pad' ); }
		else {
			bbDef = self.defineBoundingBox ( 7, DexterSim2.HiRes.finger,
							'TI1-420-002_ParallelGripperDynamicFinger_v2' ); }
		DexterSim2.HiRes.finger.position.x = self.ee.Finger.value;
		//	The finger is not a direct child of link6. So ...
		let L6w = DexterSim2.HiRes.link6.matrixWorld;	//	link6 wrt world
		let Fw = DexterSim2.HiRes.finger.matrixWorld;	//	finger wrt world
		let nL6w = new THREE.Matrix4();
			nL6w.getInverse ( L6w );
		let Fl6 = nL6w.multiply ( Fw );		//	finger wrt link6
		config.finger = { obj:		DexterSim2.HiRes.finger,
	...
	self.importDexterFromGltf = function() {
		...
		const gltfLoader = new THREE_GLTFLoader(); //fry mod
	...
	self.graspBlock = function ( blkName ) {
		...
		DexterSim2.HiRes.link6.add ( block );
	...
	self.checkScriptStatementBeingDone = function ( jvAbsMax ) {
        ...
        const sW = 'App checkScriptStatementBeingDone()';
	...
	self.updateJointPositions = function ( jc, jv ) {
		...
		if ( DexterSim2.HiRes.link1 && self.bHiResMoveEnabled ) {
			DexterSim2.HiRes.link1.rotation[axis] = jc[0]; }
		a = Math.abs ( jv[0] );
		if ( a > jvAbsMax ) {
			jvAbsMax = a; }
	
		axis = self.jt['J2'].axis;
		if ( DexterSim2.HiRes.link2 && self.bHiResMoveEnabled ) {
			DexterSim2.HiRes.link2.rotation[axis] = jc[1]; }
		a = Math.abs ( jv[1] );
		if ( a > jvAbsMax ) {
			jvAbsMax = a; }
	
		axis = self.jt['J3'].axis;
		if ( DexterSim2.HiRes.link3 && self.bHiResMoveEnabled ) {
			DexterSim2.HiRes.link3.rotation[axis] = jc[2]; }
		a = Math.abs ( jv[2] );
		if ( a > jvAbsMax ) {
			jvAbsMax = a; }

		axis = self.jt['J4'].axis;
		if ( DexterSim2.HiRes.link4 && self.bHiResMoveEnabled ) {
			DexterSim2.HiRes.link4.rotation[axis] = jc[3]; }
		a = Math.abs ( jv[3] );
		if ( a > jvAbsMax ) {
			jvAbsMax = a; }

		axis = self.jt['J5'].axis;
		if ( DexterSim2.HiRes.link5 && self.bHiResMoveEnabled ) {
			DexterSim2.HiRes.link5.rotation[axis] = jc[4]; }
		a = Math.abs ( jv[4] );
		if ( a > jvAbsMax ) {
			jvAbsMax = a; }

		axis = self.jt['J6'].axis;
		if ( DexterSim2.HiRes.link6 && self.bHiResMoveEnabled ) {
			DexterSim2.HiRes.link6.rotation[axis] = jc[5]; }
		a = Math.abs ( jv[5] );
		if ( a > jvAbsMax ) {
			jvAbsMax = a; }
		...
		if ( DexterSim2.HiRes.finger && self.bHiResMoveEnabled ) {
			DexterSim2.HiRes.finger.position[axis] = self.ee.Finger.value; }
	...
	self.render3D = function() {
		...
		let jTgtPos = [];		//	Joint target values.
         //this is the fn that gets hacked to set the joint positions
         //from Fry SimUtils.js code from render_multi_frame.
         //BUT we have to get deltatime to pass in to that fn (ms_per_frame) from here.
		...
		let curGround = {};
		let curBlocks = [];
         //fill up the above 5 arrays with the info from the actua dynamics simulation based on
         //the deltaTime. stepSimulation2 already know about the blocks in the scene
         //created elsewhere so we don't have to pass those in,
         //but we DO get their 'updated position and oriention back in the curBlocks variable.
        ...
        //now feed curBlocks into the the renderer.
		self.updateBlockPositions ( curBlocks ); //renders the moved blocks.
		...
		//Somewhere else is some global (really Sim2) variable which
		//contains an array of all the pairs of blocks that have collised.
		self.checkCollisions();

		//	When JavaScript is ready, do another frame.
		//todo: wrap around this call "If(there are active Jobs) { to the below call }
		// then when a job STARTS (and we are not already running, sim2.
		//call window.requestAnimationFrame ( self.render3D ) to start it all off.
