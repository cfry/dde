...
function eye_suggestion_string(eye_suggest_result){
	...
	if(eye_suggest_result[1][0] != 0 || eye_suggest_result[1][1] != 0){
    ...
    return "Eye is accaptable. Wait for completion.<br/>"
}

/* fry rewrite of James code above
function eye_suggestion_string(eye_suggest_result){
    if     (eye_suggest_result[0] == 1){ return "Move optical block closer to code disk.<br/>"}
    else if(eye_suggest_result[0] > 1){ return "Move optical block away from code disk.<br/>" }
    //below here eye_suggest_result[0] < 1
...
function find_perfect_center(xy_data){
	...
	let mesh = generate_mesh(x_bounds, y_bounds, 4)
    //out(mesh)
    //out(xy_data)
    let inside_mesh = []
...
function center_calc(contour, inside_mesh){
    ...
    for(let i = 0; i < inside_mesh.length; i++){
        ...
        mesh_obj.push({x: inside_mesh[i][0], y: inside_mesh[i][0], ave: Vector.average(temp_dists), min: Vector.min(temp_dists), idx: i})
    }
    let top_4_ave = []
    let top_4_min = []
    if(mesh_obj.length > 4){
