...
import https     from "https" //see https://www.golinuxcloud.com/create-https-server-with-node-js/
...
var http_server = https.createServer(options, async function (req, res) {
...
http_server.listen(443) //was 80
console.log("listening on port 443 with server: " + http_server)
...
//__________For DDE to MONITOR the Job Engine. NOT the main websocket connection
//https://www.npmjs.com/package/ws
console.log("now making wss for Job Engine monitor");
const wss_for_job_engine = new WebSocketServer({port: 3001});    //server: http_server });
console.log("done making wss: " + wss_for_job_engine);



// Web Socket Proxy to DexRun raw socket
wss_for_job_engine.on('connection', function(the_ws, req) {
  console.log("\n\nwss for Job Engine got connection: " + the_ws)
...
//______main WebSocket code for DDE IDE_______
...
//dde4 cloud, jun 2023, see https://www.npmjs.com/package/ws#external-https-server ,
//example under the header: External HTTP/S server

//var browser = new WebSocketServer({ port:3000 }) //old
...
function define_web_socket_event_handlers(a_web_socket){
    ...
    a_web_socket.on('close', function (data) {
        ...
        //}
        //dexter_net_socket.close() //not defined.
        dexter_net_socket.connected = false
        dexter_net_socket.removeAllListeners()
        dexter_net_socket.end()
        dexter_net_socket.destroy() //was included but maybe shouldn't be. Never called in DDE3 or 4.
    });
