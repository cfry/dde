...
function handle_keypoints_opencv(vals){
    if(vals.clicked_button_value == "init"){ // Clicked button value holds the name of the clicked button.
        ...
        let context = src_image_id.getContext('2d')
        context.drawImage(video_id, 0, 0, 320, 240) //grab a still from video_id and show in canvas_id
    }
    else if (vals.clicked_button_value == "sort_by_x")    { sbd_sort_by = "x" }
...
function sbd_data_table_header() {
    return "Keypoints Data (1 blob per row) <table><tr>" +
...
show_window({
    content:
     ...
     <video  id="video_id"  width="320" height="240" style="border-style:solid; border-width:1px; margin:3px;" autoplay></video>
     <canvas id="src_image_id" width="320" height="240" style="border-style:solid; border-width:1px; margin:3px;""></canvas><br/>
