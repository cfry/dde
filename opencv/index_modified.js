...
let qvga_ocv = {width: {exact: 320}, height: {exact: 240}};

let vga_ocv = {width: {exact: 640}, height: {exact: 480}};

let resolution = window.innerWidth < 640 ? qvga_ocv : vga_ovc;
...
function startCamera() {
  ...
  video.addEventListener("canplay", function(ev){
    if (!streaming) {
      height_ocv = video.videoHeight;
      width_ocv = video.videoWidth;
      video.setAttribute("width", width_ocv);
      video.setAttribute("height", height_ocv);
      streaming = true;
...
function startVideoProcessing() {
  ...
  stopVideoProcessing();
  src = new cv.Mat(height_ocv, width_ocv, cv.CV_8UC4);
  dstC1 = new cv.Mat(height_ocv, width_ocv, cv.CV_8UC1);
  dstC3 = new cv.Mat(height_ocv, width_ocv, cv.CV_8UC3);
  dstC4 = new cv.Mat(height_ocv, width_ocv, cv.CV_8UC4);
...
function inRange(src) {
  ...
  let low = new cv.Mat(height_ocv, width_ocv, src.type(), lowScalar);
  let high = new cv.Mat(height_ocv, width_ocv, src.type(), highScalar);
...
function adaptiveThreshold(src) {
  let mat = new cv.Mat(height_ocv, width_ocv, cv.CV_8U);
...
function bilateralFilter(src) {
  let mat = new cv.Mat(height_ocv, width_ocv, cv.CV_8UC3);
...
function sobel(src) {
  var mat = new cv.Mat(height_ocv, width_ocv, cv.CV_8UC1);
...
function contours(src) {
  ...
  dstC3.delete();
  dstC3 = cv.Mat.ones(height_ocv, width_ocv, cv.CV_8UC3);
...
function stopCamera() {
  if (!streaming) return;
  stopVideoProcessing();
  document.getElementById("canvasOutput").getContext("2d").clearRect(0, 0, width_ocv, height_ocv);
  video.pause();
