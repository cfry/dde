...
import SerialPort from 'serialport' //"To use SerialPort, you must call serial_port_init() first."
...
export var serial_port_path_to_info_map = {}
...
export var serial_port_path_to_port_map = {}

export function serial_get_or_make_port(port_path, port_options, open_callback){
...
export function serial_devices(){ //todo get serial devices on server, but also on client?
    if(platform === "dde") {
...
export function serial_devices_async(callback = serial_devicies_async_callback_default){
...
export function serial_connect_low_level(port_path,
                                  port_options,
                                  capture_n_items=1,
                                  item_delimiter="\n",
...
export function serial_send_low_level(port_path, content){
...
export function serial_connect(port_path, port_options, simulate=null, capture_n_items=1, item_delimiter="\n",
                        trim_whitespace=true,
...
export function convertArrayBufferToString(buf, content_length=null) {
...
//content is a string
export function serial_send(instruction_array, port_path, simulate=null, sim_fun) {
...
export function serial_flush(port_path){
    let info = serial_port_path_to_info(port_path)
...
export function serial_disconnect(port_path){
    let info = serial_port_path_to_info_map[port_path]
    if (info){
        if((info.simulate === false) || (info.simulate === "both")) {
...
export function serial_disconnect_all(){
