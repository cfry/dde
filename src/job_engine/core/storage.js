...
export function persistent_save(){
...
export function persistent_set(key, value){
    persistent_values[key] = value
    persistent_save()
 }

//returns undefined if key doesn't exist
export function persistent_get(key="get_all"){
    if (key == "get_all") { return persistent_values }
    else { return persistent_values[key] }
}


export function persistent_remove(key) {
...
export function dde_init_dot_js_initialize() {
    if(!file_exists("")){
...
export function read_file(path, encoding="utf8"){
...
export var file_content = read_file
...
export function read_file_async(path, encoding="utf8", callback){
...
export function choose_file(show_dialog_options={}) { //todo busted in dde4, see dde_to_do_list.
...
export function choose_folder(show_dialog_options={}) { //todo busted in dde4, see dde_to_do_list.
...
export function choose_file_and_get_content(show_dialog_options={}, encoding="utf8") {
...
export function choose_save_file(show_dialog_options={}) { //todo document
...
export function write_file(path, content, encoding="utf8"){
    if (path === undefined){
...
export function write_file_async(path, content, encoding="utf8", callback=write_file_async_default_callback){
    if (path === undefined){
        if (Editor.current_file_path == "new buffer"){
            dde_error("Attempt to write file but no filepath given.")
        }
...
export function append_to_file(path, content, encoding="utf8"){
    path = make_full_path(path)
...
export function copy_file_async(source_path, destination_path, callback=null){
    if(!callback) {
...
export function copy_folder_async(source_folder, destination_folder, callback=copy_folder_async_default_callback){
...
//is syncrhonous
export function make_folder(path){
...
export function folder_separator(){
...
export function convert_backslashes_to_slashes(a_string){
...
export function adjust_path_to_os(path){
...
export function make_full_path(path, adjust_to_os=true){
...
//until dde is booted.
export function node_server_supports_editor(dexter_instance){
...
export function load_files(...paths) {
...
export function is_folder(path=dde_apps_folder){
    return fs.statSync(path).isDirectory()
}

//could have been called list_folder
export function folder_listing(folder=dde_apps_folder, include_files=true, include_folders=true, include_folder_name=true){
...
export function get_latest_path(path){
...
export function make_unique_path(path){
