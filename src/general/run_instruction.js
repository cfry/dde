globalThis.RunInstruction = class RunInstruction {
    static move_all_joints_args_string(vals, relative = false) {
        let result = ""
        for (let j = 1; j <= 7; j++) {
            let id_str = "J" + j + "_id"
            let val = vals[id_str]
    ...
    static move_to_args_string(vals) {
        const xyz = "[" +
    ...
    static show_run_instruction_dialog_cb(vals) {
        ...
        if (vals.clicked_button_value == "Insert Dexter Def") {
            ...
            instr_src = "Dexter.move_to(" + RunInstruction.move_to_args_string(vals) + ")"
            ...
            const arg = RunInstruction.move_all_joints_args_string(vals) //[vals.J1_id, vals.J2_id, vals.J3_id, vals.J4_id, vals.J5_id]
            instr_src = "Dexter.move_all_joints(" + arg + ")"
        } else if (vals.clicked_button_value == "move_all_joints_relative") {
            const arg = RunInstruction.move_all_joints_args_string(vals, true)
            ...
            if (sel_instr_name == "Dexter.move_to_straight") {
                instr_src = "Dexter.move_to_straight(" + RunInstruction.move_to_args_string(vals) + ")"
                ...
                const arg = RunInstruction.move_all_joints_args_string(vals)
                instr_src = "Dexter.move_all_joints(" + arg + ")"
            ...
            instr_src = RunInstruction.src_of_run_instruction()
        } else if (vals.clicked_button_value == "insert") {
            Editor.insert(RunInstruction.src_of_run_instruction() + "\n")
            return
        } else if (vals.clicked_button_value == "job") {
            let src = vals.mode_name
            let src_of_run_ins = RunInstruction.src_of_run_instruction()
        ...
        RunInstruction.onchange_run_instruction_src_aux(instr_name)
    ...
    static onchange_run_instruction_src() {
        const instr_name = dexter_instructions_id.value
        RunInstruction.onchange_run_instruction_src_aux(instr_name)
        run_src_id.value = ""
    }

    static onchange_run_instruction_src_aux(instr_name) {
        const instr_fn = value_of_path(instr_name)
        const instr_params = Utils.trim_all(Utils.function_params(instr_fn))
        var font_size = "14px"
        ...
        if (details_elt) {
            DocCode.open_doc(details_elt)
        }
    }

    static src_of_run_instruction() {
    ...
    static make_set_parameter_name_html() {
    ...
    static make_dexter_instructions_html() {
    ...
    static J5_direction_to_xyz_html() {
    ...
    static J5_direction_to_angles_html() {
        var new_angles = [0, 0]
        if (globalThis.J5_direction_xyz_x_id) {
            ...
            if ((xyz_array[0] == 0) && (xyz_array[1] == 0) && (xyz_array[2] == -1)) {
                ...
                try {
                    new_angles = Kin.dir_xyz_to_angles(xyz_array)
                } catch (err) {
    ...
    static insert_dexter_def(dex_name) {
    ...
    static show_run_instruction_dialog() {
        show_window({
            content:
                "Send instruction to: " +
                RunInstruction.make_robots_select_html() +
                " &nbsp;&nbsp;&nbsp;<input type='button' value='Insert Dexter Def' " +
                "title='Insert the definition of the selected Dexter into the editor.' " +
                "/>" +
                `<br/>
         Change mode to:` + RunInstruction.make_modes_select_html() +
         ...
         <tr><td>X:<input name="X_id"  type="number" value=0 title="Blank means no change." step="0.01" style="width:90px;"/><br/>
             ...
             <td>Name: ` + RunInstruction.make_set_parameter_name_html() + `</td>
         </tr>
         ...
         <tr><td style="font-size:11px;"><b>J5_direction</b><br/>
                   ...
                   <input type="radio" name="angles_xyz" value="xyz"    onclick="RunInstruction.J5_direction_to_xyz_html()" id="angles_xyz_xyz_id"/>xyz
         ...
         <input type="button" value="run" title="Eval this source to make an instruction,&#013;then run it." style="margin:5px;"/>` +
                RunInstruction.make_dexter_instructions_html() +
         ...
         `,
            ...
            width: 550,
            height: 610,
            callback: "RunInstruction.show_run_instruction_dialog_cb"//handle_run_instruction
        })
        ...
        RunInstruction.J5_direction_to_angles_html()
        ...
        if (sel_text.length > 0) {
            run_src_id.value = sel_text
        }
    } //end of show_run_instruction_dialog
} //end of class
