...
globalThis.TrackHand = class TrackHand {
    ...
    static async init() { //was getVisionStuff()
        ...
        //    out("called onLoadeddata")
        //}

        navigator.mediaDevices.getUserMedia({ video: true }).then((stream) => {
            trackhand_webcam_id.srcObject = stream;
            //trackhand_webcam_id.addEventListener("loadeddata", predictWebcam);
            trackhand_webcam_id.addEventListener("loadeddata", TrackHand.capture_data)
        });
    }

    static enable_run = true
    static stop() {
        /*if (this.intervalID) {
    ...
    static async capture_data() { //was predictWebcam
        let predictions = await TrackHand.model.detect(trackhand_webcam_id);
        out(predictions)
        if(this.enable_run) {
            window.requestAnimationFrame(TrackHand.capture_data);
        }
        else { //stop the recursion but the next time we call capture_data, it will be enabled.
