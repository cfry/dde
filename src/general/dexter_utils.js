...
class DexterUtils {
    ...
    static make_ping_a_dexter_select(){
    ...
    static ping_a_dexter_handler(vals){
        ...
        if(!menu_label) { out("No ip_address selected.") }
            ...
            if(vals.clicked_button_value == "Ping") {
                ...
                DexterUtils.ping_a_dexter_make_dex_def(ip_address)
            }
        }
    }

    static ping_a_dexter_make_dex_def(ip_address){
        let new_dex_name
        for(let i = 1; i < 255; i++){
    ...
    static ping_a_dexter_ongoing = false

    static ping_a_dexter_wildcard(ip_address_with_wildcard){
        if(ip_address_with_wildcard.endsWith("**")) {
            ...
            this.ping_a_dexter_ongoing = true
            this.ping_a_dexter_double_wildcard_aux(ip_address_with_wildcard, 0, 0)
        }
        else {
            ...
            this.ping_a_dexter_ongoing = true
            this.ping_a_dexter_wildcard_aux(ip_address_with_wildcard, 0, 0)
        }
    }

    static ping_a_dexter_wildcard_aux(ip_address_with_wildcard, host = 0, connected_count=0, stop_after_nine=true){
        if (host < 10) {
            ...
            ping.sys.probe(ip_address, function(isAlive){
                ...
                else if(this.ping_a_dexter_ongoing){
                    this.ping_a_dexter_wildcard_aux(ip_address_with_wildcard, host + 1, connected_count)
                }
                else {
    ...
    static ping_a_dexter_double_wildcard_aux(ip_address_with_wildcard, host = 0, connected_count=0, stop_after_nine=true){
        if (host < 100) {
            let base_address = ip_address_with_wildcard.substring(0, ip_address_with_wildcard.length - 2)
            let ip_address
            ...
            ping.sys.probe(ip_address, function(isAlive){
                ...
                if(host == 99) {
                    ...
                    this.ping_a_dexter_double_wildcard_aux(ip_address_with_wildcard, host + 1, connected_count)
                }
                else {
                    out("ping scan stopped after: " + ip_address + "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" + connected_count + " connected host(s) found.")
                }
    ...
    static ping_a_dexter_scan_aux(net_address, host = 0, connected_count=0){
        if (host < 10) {
            ...
            ping.sys.probe(ip_address, function(isAlive){
                ...
                if(host == 255) {
                    ...
                    this.ping_a_dexter_scan_aux(net_address, host + 1, connected_count)
                }
                else {
                    out("ping scan stopped after: " + ip_address + "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" + connected_count + " connected host(s) found.")
                }
    ...
    static ping_a_dexter_scan(net_address) {
      out("Pinging addresses from " + net_address + 0 + " through " +  net_address + 255 + " ...")
        this.ping_a_dexter_ongoing = true
        this.ping_a_dexter_scan_aux(net_address, 0, 0)
    }
        ...
        show_window({title: "Ping a Dexter",
                     content: `Test to see if a Dexter or other ip_address<br/>
                               ...
                               "<i>Type in, or select, an ip address.</i><br/>" +
                                 ...
                                 "<p/>" +
                            this.make_ping_a_dexter_select() +
                     ...
                     height: 275,
                     width:  480,
                     callback: "DexterUtils.ping_a_dexter_handler"
                    })
    }
    ...
    static onchange_dexter_default(){
    ...
    static default_dexter_full_name(){
    ...
    static add_dexter_to_dexter_default_menu(a_dexter_or_dexter_name){
        let dex_name
        ...
        else { shouldnt("DexterUtils.add_dexter_to_dexter_default_menu passed: " + a_dexter_or_dexter_name +
    ...
    static show_dexter_start_options_cb(vals){
    ...
    static show_dexter_start_options_write_cb(err){
        let path = this.dexter_start_options_path()
        if(err){
    ...
    static show_dexter_start_options_lines = null

    static show_dexter_start_options_aux(file_content){
        ...
        show_window({title: "Dexter." + Dexter.default.name + " Start Options",
                     ...
                     callback: "DexterUtils.show_dexter_start_options_cb"
                    })
    }
}
