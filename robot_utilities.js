...
function ping_host(ip_address = "127.0.0.1"){
    ping.sys.probe(ip_address,
                    function(isAlive){
                        out("ping: Is " + ip_address + " connected? " + isAlive)
                    }
    )
}
...
function make_ping_a_dexter_select(){
    ...
    let result =  '<div name="ping_a_dexter_id_address_id" class="combo_box" style="display:inline-block;vertical-align:middle;">' +
                   '<option></option> ' +
...
function ping_a_dexter_scan(net_address, host = 0, connected_count=0) {
  out("Pinging addresses from " + net_address + 0 + " through " +  net_address + 255 + " ...")
  ping_a_dexter_ongoing = true
  ping_a_dexter_scan_aux(net_address, host, connected_count)
}
...
function ping_a_dexter(){
    show_window({title: "Ping a Dexter",
                 content: `Test to see if a Dexter or other ip_address<br/>
                           ...
                           "<i>Type in or select an ip address.<br/>" +
                               "If the address is of the format:<br/>" +
                               "123.45.6. then addresses <br/>" +
