...
var Socket = class Socket{
    ...
    static robot_ready(){
        let ping_proxy = require('ping-proxy')
        if (this.robot instanceof Dexter) {
            pingProxy({proxyHost: this.robot.ip_address,
                    proxyPort: this.robot.port},
                function(err, options, statusCode) {
                    if (err) {
                        warning(err);
                    }
                    else {
                        out(options, statusCode)
                    }
                })
        }
        else { out("no dexter to wait for") }
    }
